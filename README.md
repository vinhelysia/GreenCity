...
Working today: email/password auth (Argon2id, opaque DB-backed sessions), and a scrap-marketplace vertical slice — sellers submit scrap with a photo, admins quote within a published price band, sellers accept, listings go live, and subscribed buyers reserve (one winner per listing, enforced transactionally). The code includes a payOS VietQR checkout for buyer eligibility — a one-time 50.000đ / 30-day bank transfer, no auto-renewal, verified server-side by an HMAC-signed webhook — but it is only reachable once `PAYOS_CLIENT_ID`, `PAYOS_API_KEY`, `PAYOS_CHECKSUM_KEY` and `PUBLIC_WEB_URL` are configured (`PUBLIC_API_URL` is not required for checkout — it only tells you the webhook address to register with payOS), and it has **not yet been exercised against a real payOS merchant account**. Stack: NestJS API, Next.js web, Prisma, native local PostgreSQL + PostGIS, local filesystem storage, console mail.

Not implemented, and not faked: seller reward payout, cleanup assignment, field collection, before/after evidence, and completion verification. payOS publishes no sandbox, so the integration stays unexercised until a merchant account exists and a real checkout/webhook round trip has been observed against production — see `tasks/plan.md`. The webhook address must be registered with payOS separately; it is not sent per request.
